...
def json2features(data, tokenizer, is_training=False, repeat_limit=3, max_query_length=64,
    ...
    train_data = data
    def _is_chinese_char(cp): # 判断是否是中文
...
def predict(model, batch_size, eval_examples, eval_features, device):
    RawResult = collections.namedtuple("RawResult",
                                       ["unique_id", "start_logits", "end_logits"])
    ...
    for input_ids, input_mask, segment_ids,example_indices in eval_dataloader:
        ...
        for start, end, input_id in zip(start_logits, end_logits,input_ids):
            pred_anser_id = input_id[start:end + 1]
            pred_answer_letter = tokenizer.decode(pred_anser_id)
            pred_answer.append(pred_answer_letter)
...
model_path = './model_hub/albert-chinese-large-qa'
...
tokenizer = AutoTokenizer.from_pretrained(model_path, **tokenizer_kwards)

bert_config = AutoConfig.from_pretrained(model_path)

model = AutoModelForQuestionAnswering.from_pretrained(model_path,
                                                   **{'config':bert_config}).to('cuda:0')

question = '《战国无双3》是由哪两个公司合作开发的？'
context = r'《战国无双3》（）是由光荣和ω-force开发的战国无双系列的正统第三续作。本作以三大故事为主轴，分别是以武田信玄等人为主的《关东三国志》，织田信长等人为主的《战国三杰》，石田三成等人为主的《关原的年轻武者》，丰富游戏内的剧情。此部份专门介绍角色，欲知武器情报、奥义字或擅长攻击类型等，请至战国无双系列1.由于乡里大辅先生因故去世，不得不寻找其他声优接手。从猛将传 and Z开始。2.战国无双 编年史的原创男女主角亦有专属声优。此模式是任天堂游戏谜之村雨城改编的新增模式。本作中共有20张战场地图（不含村雨城），后来发行的猛将传再新增3张战场地图。但游戏内战役数量繁多，部分地图会有兼用的状况，战役虚实则是以光荣发行的2本「战国无双3 人物真书」内容为主，以下是相关介绍。（注：前方加☆者为猛将传新增关卡及地图。）合并本篇和猛将传的内容，村雨城模式剔除，战国史模式可直接游玩。主打两大模式「战史演武」&「争霸演武」。系列作品外传作品'
...
try:
    ...
    print("length",len(example),len(feature))
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    result = predict(model,len(example),example,feature,device)
    # 结果还要处理一下
    # result = evaluate(model,len(example),example,feature,'cuda:0')[0]['QUERY_0']
